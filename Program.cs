...
namespace WebApiClient
{
    class Program
    {
        ...
        {
            //String createTable = File.ReadAllText(@"./Queries/CREATE_TABLE.sql");
            String createTable = @"
            IF NOT EXISTS (SELECT * FROM sysobjects WHERE Name = 'weather_records')
                CREATE TABLE weather_records (
                    RecordID    INT             IDENTITY(1, 1)  PRIMARY KEY,
            ...
            {
                connection.Open();
                SqlDataReader reader = createTableCommand.ExecuteReader();
            }
            catch (System.Exception ex)
            {
                Console.WriteLine(ex.ToString());
        ...
        {
            // String insertInto = File.ReadAllText(@"./Queries/INSERT_INTO_NEW.sql");
            // To test reading from JSON: SELECT * FROM OPENROWSET (BULK '/data/out/<dd-mm-yyyy>.json', SINGLE_CLOB) AS ROW_SET;
            // To test reading from JSON with dynamic SQL
            /*
                DECLARE @SQL NVARCHAR(MAX)
                SET @SQL = N'
                    SELECT * FROM OPENROWSET (BULK ''' + @FilePath + ''', SINGLE_CLOB) AS ROW_SET;
                '
                EXEC(@SQL)
            */
            ...
            -- Inserting JSON into DB
                ...
                SET @SQL = N'
                    INSERT INTO weather_records (Area, Forecast, SqlStartTime, SqlEndTime)
                    SELECT *
                    FROM OPENROWSET (BULK ''' + @FilePath + ''', SINGLE_CLOB) AS ROW_SET
                    WITH (
                        Area VARCHAR(255),
                        Forecast VARCHAR(255),
                        SqlStartTime DATETIME,
            ...
            */
            SqlCommand insertIntoCommand = new SqlCommand(insertInto, connection);
            try
            {
                connection.Open();
                SqlDataReader reader = insertIntoCommand.ExecuteReader();
            }
            catch (System.Exception ex)
            {
                Console.WriteLine(ex.ToString());
        ...
        {
            //String readFrom = File.ReadAllText(@"./Queries/SELECT_TABLE.sql");
            String readFrom = @"
                SELECT 
                    RecordID AS RID, 
                    Area AS AREA, 
                ...
                FROM weather_records;
            ";
            SqlCommand readFromCommand = new SqlCommand(readFrom, connection);
            try
            {
                connection.Open();
                SqlDataReader reader = readFromCommand.ExecuteReader();
                while (reader.Read())
                {
                    Console.WriteLine("{0, -5}{1, -40}{2, -45}{3, -25}{4, -25}", 
                        reader["RID"].ToString(),
